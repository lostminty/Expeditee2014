/*jslint undef: true, nomen: true, plusplus: true, newcap: true, browser: true, devel: true, passfail: false, "white": false */
 ...
 * This file has been modified from the original Arc90 Version
 * Modifications by Nathaniel Watson
...
var readability = {
    ...
    maxPages:    10, /* The maximum number of pages to loop through before we call it quits and just show a link. */
    ...
    regexps: {
        ...
        extraneous:            /print|archive|comment|discuss|e[\-]?mail|share|reply|all|login|sign|single|cite/i,
    ...
    init: function() {
        ...
        if(!articleContent) {
            ...
            articleContent.innerHTML = "<p>Sorry, we were unable to parse this page for content.</p>";
        ...
        document.body.className = "expeditee-serif";
    ...
    **/
    ...
    getArticleTools: function () {
        ...
        articleTools.innerHTML = 
            "<a href='#' onclick='window.location.reload()' title='Reload original page' id='reload-page'><i class='fa fa-undo fa-2x'></i></a>" +
            "<a href='#' onclick='readability.zoomText(1); return false' title='Increase text size' id='zoom-in'><i class='fa fa-plus fa-2x'></i></a>" +
            "<a href='#' onclick='readability.zoomText(-1); return false' title='Decrease text size' id='zoom-out'><i class='fa fa-minus fa-2x'></i></a>" +
            "<a href='#' onclick='readability.swapFontStyle(); return false' title='Swap font style' id='swap-font'><i class='fa fa-font fa-2x'></i></a>";

        ;
        return articleTools;
    },
    
    swapFontStyle: function () {
      if(document.body.className.toLowerCase() === "expeditee-serif"){
          document.body.className = "expeditee-sans";
      } else {
          document.body.className = "expeditee-serif"
      }
    },
    
    zoomText: function (factor) {
        overlay = document.getElementById('readOverlay');
        if(overlay.style.fontSize == "") {
            overlay.style.fontSize = "1.0em";
        }
        
        overlay.style.fontSize = parseFloat(overlay.style.fontSize) + (factor * 0.1) + "em";
    },
    ...
    getArticleFooter: function () {
        ...
        "<div id='rdb-footer-wrapper'>",
             "<div id='rdb-footer-right'>",
                 "<span class='version'>Based on Readability version " + readability.version + "</span>",
             "</div>",
        "</div>"].join('');
